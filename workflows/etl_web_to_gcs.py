...
def transform(data: pd.DataFrame) -> pd.DataFrame:
    """Basic transformation step. Merges columns CRASH DATE and CRASH TIME into a new column CRASH DATETIME and
    transforms ZIP CODE column to int.
    """
    def transform_zipcode(zip_code):
        if isinstance(zip_code, str):
            zip_code = zip_code.strip()
            if zip_code:
                return int(zip_code.strip())
            return -1
...
@task()
def write_local(data: pd.DataFrame, data_dir: str = 'raw', dataset_filename: str = 'nyc_mvc') -> Path:
